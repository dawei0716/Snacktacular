...
class Spot{
    ...
    func saveData(completed: @escaping (Bool)->()){
        ...
        //if we have saved a record, we'll have a docID
        if self.documentID != ""{
            ...
            var ref: DocumentReference? = nil //let firestore crete new DocID
            ref = db.collection("spots").addDocument(data: dataToSave) { error in
                if let error = error{
                    print("ERROR: creating new document \(error.localizedDescription)")
                    return completed(false)
                }else{
                    print("new document created: \(ref?.documentID ?? "unknown")")
                    completed(true)
                }
            }
        }
